...
export async function loader({ request }: { request: Request }): Promise<LoaderData> {
  ...
  // Fetch Open-Meteo daily forecast for 7 days
  ...
  try {
    ...
    degraded = true;
    // Create placeholder 7 days starting today with null metrics
  ...
  return { role, city, coords, days, tasks: visible, degraded };
}
